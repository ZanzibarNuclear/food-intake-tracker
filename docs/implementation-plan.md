...
A **first prototype** exists. Treat it as reusable foundation, not the product.
...
| `server/data/seed.json` | **Stale** (32 foods / 23 meals) | **Regenerate** from 2026-06-28 workbook (97 / 55 / 4) |
| `docs/postgres-schema.sql` | Done | **Keep** — apply via `npm run db:setup` |
| `app.vue` (4-tab monolith) | Workbook-like UI, no charts | **Replace** with component-based screens |
| `composables/useTrackerStore.ts` | localStorage overrides server | **Delete** — DB-only persistence |
| `server/utils/repository.ts` | Basic CRUD | **Replace** with full repository |
| `server/api/*.ts` | GET tracker + POST only | **Extend** — search, edit, delete, settings |
| Charts, catalog seed, AI, favorites | Not built | **Build** per phases below |

**Known bug to fix in Phase 1:** `useTrackerStore` loads `localStorage` on mount and overwrites server data. v1 uses Postgres only.
...
- Target **~300 common foods** with calories and protein per serving (expand toward 500 later).
- Search ranks **personal foods first**, then catalog.
- Filter: All / My foods / Catalog.
- Nutrition score on catalog items: default **5**; user adjusts when copying or on first log.
...
knownFood        = food name exists in catalog
```
...
- **meal_entries** — `food_id`, quantity, date, meal type.
...
1. Add `tools/fetch_system_foods.py` (or Node script) to pull a curated subset from [USDA FoodData Central](https://fdc.nal.usda.gov/) — Foundation Foods and/or SR Legacy.
2. Normalize to app food shape: name, serving description, calories, protein per serving.
3. Output `server/data/system-foods.json` with `is_system_seed: true`, `source: 'usda'`, `nutrition_score: 5`.
4. `npm run db:setup` loads schema + both seeds; idempotent upsert (personal by name, catalog by stable slug/id).
...
### Phase 4 — System food catalog

- Build `tools/fetch_system_foods.py` and `server/data/system-foods.json` (~300 items).
- Extend `db:setup` to load catalog.
- Foods screen: filter All / My / Catalog; catalog rows read-only with “Use” / “Copy to my foods”.
...
| Question | Default |
|----------|---------|
| Catalog size for v1 | ~300 USDA items |
| Nutrition score on catalog | 5 until user edits |
